...
4月始まり。**ラマダンはトルコ盤面と同じやり方で8月の枠に入れている**
(太陰暦で年ごとに月がずれるため、便宜上固定。ずれることは英語ほか4言語の
`f`にも書いてあり、隠していない)。ほかに王位記念日(7月30日)・独立記念日/
緑の行進(11月18日/6日)・ヤンナイル(アマジグ暦新年、1月中旬、グレゴリオ暦で
固定)など、日付が固定された祝祭も選んだ。
...
## team-lead 指摘への対応(2巡目)

1. **エッサウィラの「サン・マロを手本に」**は観光案内の簡略化だったため、
   「アヴィニョン出身の軍事技術者テオドール・コルニュがヴォーバン式の
   星形稜堡の伝統に則って設計した」を断定できる事実として書き直し、
   サン・マロへの言及は「しばしばなぞらえられるが、裏付けのある手本という
   より観光案内の慣用句に近い」という形に弱めた(4言語とも修正)。
2. **地方あたり4都市は薄い**という指摘を受け、東部(est)に**ジェラダ**を
   追加して4→5都市にした。国営炭鉱が2000年代初頭に閉山したあとも
   非公式に採掘を続ける鉱夫が絶えず、2017年12月の死亡事故を機に
   数か月続いた抗議運動(通称「ヒラク」)がいまも地元の記憶に残っている、
   という筋で書いた(観光地ではない、実質のある町を選んだ)。
   路線はウジダ―フィギッグの直行(522px)をウジダ―ジェラダ―フィギッグの
   中継に置き換え、**39都市42路線**になった。全地方が5以上
   (rif7/atl8/cen5/atm5/sud9/est5)。
3. **ラマダンをトルコ盤面と同じ方式で追加**(上記6a参照)。8月の枠に
   便宜上固定し、太陰暦で実際の月がずれることを4言語の`f`(補足事実)に
   明記した(隠していない)。ナファル(夜明け前に太鼓で家々を起こして回る
   人)という具体的な習わしを書いた。
4. **背景の密度を1枚ずつ報告**(下記)。30を下回っていた2枚
   (gorge29・coastdesert29)に要素を足し、全22枚が33以上になった。

## 自分で確かめたこと

- `node -e` で `buildMoroccoContent()` を呼び、`cities`/`edges`/`quiz`/`moneyEvents`/
  `items`/`doom`/`seasons`/`regions`/`marks`/`bg`/`styles` の件数を確認:
  **都市39・路線42・クイズ36・出来事20・アイテム9・厄災7・季節12・地方6・
...
- **都市の陸地判定**: 39都市すべての座標が `geography.mjs` の海岸線ポリゴンの
  ...
  投影の東端近くまで国境を張り出させて直した。ジェラダ追加後も再確認し、
  全39件が陸地の内側・投影範囲内であることを確認済み。
- **`seg` の実測**: 42路線すべての投影後距離を計算し、最長はアガディール―
  タンタン469px・ジェラダ―フィギッグ463px(ジェラダ追加でウジダ―フィギッグの
  直行522pxを分割したぶん、最長距離自体も縮んだ)。`seg=100` で**全路線が
  5マス以下に収まる**(5マスは2本)。実測に基づき `seg=100` を維持した。
...
- **隠れ帯(横151〜249・縦54〜152)チェック**: `getBBox()` で図形ごとの外接矩形を
  ...
  再確認で0件。
- **背景の密度(1枚ずつ)**: 22背景の要素数(`<rect|circle|ellipse|path|line|
  polygon|polyline>` タグ数)を機械計測。**平均だけでなく1枚ずつ報告する**
  よう指摘を受けたので、全件を挙げる:

  ```
  bluewalls 42 / medina 47 / chalet 40 / skala 40 / dunes 40 / strait 34 /
  andalou 41 / rifbay 39 / capital 58 / metropolis 51 / cistern 40 /
  pottery 33 / resort 41 / koutoubia 47 / ruins 42 / countryside 44 /
  mountains 36 / kasbah 45 / palmoasis 61 / gorge 37 / coastdesert 35 /
  borderdesert 35
  ```

  当初は gorge29・coastdesert29 の2枚が30を下回っていたため要素を足し、
  **最少は pottery の33**まで底上げした(平均42.2)。
  rsvg-convert でPNG化し、capital・koutoubia・cistern・kasbah・metropolis・gorge
  を目視確認済み(cisternは唯一の屋内場面で、他と雰囲気が明確に違うことを確認)。
- `mark`(32種)と `bg`(22種)は `cities.mjs` の39都市から過不足なく参照されている
...
- **地方区分と都市数**: rif7・atl8・cen5・atm5・sud9・est5。南部・砂漠が
  ...
  この地方に厚く配分した。東部(ウジダ・ジェラダ・フィギッグ・タザ・
  ベルカン)は指摘を受けて4→5に増やしたが、実在する町の選択肢自体が
  他地方より少なく、6地方の中ではいちばん薄いままである。
