...
- **海陸判定(`check-sea-routes.mjs`)**: 57路線の時点で実測いただき、
  0本(2026-08-14)。`arica–antofagasta` は海岸線側(イキケの頂点)を
  沖へ押し出して直したもので、以後の座標変更はしていません。
  **58本目の`tabatinga–belem`はこの実測の対象外**(下記「まだ確認
  できていないこと」参照)。
- **点内判定・自己交差の自作チェック**: 46都市とも陸地ポリゴンの内側、
  MAINLAND(85点)・FUEGO(10点)とも自己交差0件(繰り返し確認済み。
  ベレン追加後も再実行し、46都市全件で確認)。
- **4言語の欠け**: `cities.mjs`(46都市)・`quiz.mjs`(39問)・
...
- `node scripts/check-quiz.mjs southamerica` — 答えの位置(0:13 / 1:12 / 2:14)は
...
- 46都市・58路線・地方6区分(car4/gui6/riv6/and11/atc4/pla15)を対象にした
  本検査一式(海陸以外)。特にzodによる読み込み自体は、今回`f`欠落で
  一度落ちているので、次の焼き直しで19件のテストが通ることを
  確かめていただきたいです。
- **`check-sea-routes.mjs`を58路線で再実行してください。** 57路線までは
  0本を確認済みですが、新規の`tabatinga–belem`(58本目)はまだ本検査を
  受けていません。自作の点内判定では直線が全区間陸地ポリゴンの中でしたが、
  実際の折れ方(`edgeIndex % 2`)は反映していない簡易チェックです。
  差し戻しがあれば、他の路線と同じく端の入れ替え(添字を動かさない)で
  対応します。
...
- `buildSouthAmericaContent()` を直接呼び出して確認: 都市46・路線58・
  クイズ39・アイテム9・厄災7・季節12・出来事25・マーク46・背景46・
  地形帯9・河川3・湖沼1・ラベル11、いずれもエラー無し(ベレン追加後に
  再実行して確認)。
