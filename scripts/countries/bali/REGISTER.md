...
## 修正履歴(team-lead指摘への対応)

最初の版(32都市)を提出後、team-leadから3点の差し戻しを受け、次のとおり直した。

1. **物件価格64件(→66件、都市が1つ増えたため)の帯が狭すぎた**
   (200〜320の1.6倍しか開いていなかった)。
   `別格`(ブサキ2件、2600〜2800)/`高い`(ウルワトゥ・タナロット・ジャティルウィ・
   ウブド・ティルタエンプル計6件、1200〜1700)/`中`(州都・観光の中心・大きな寺
   22件、550〜850)/`安い`(小さな村・渡し舟・塩田など残り36件、150〜310)の
   4段に振り直した。**最終: 150〜2800(18.7倍)、利回りは全物件20.65〜20.95%
   に収まったまま**(実測値、下記参照)。振り直す過程で、慰霊碑(クタ)と
   ププタンの門(クルンクン)という2件を、悲劇の場そのものに値段を付ける形に
   なっていたため、別の実在する物件(クタ・ビーチの遊歩道/プリ・アグン・
   スマラプラ)に差し替えた(ププタン・爆弾事件そのものは `fact` の記述として残す)。
2. **ヌサペニダの「2016年に道路開通」が裏付けられなかった。**
   team-lead指示のとおり「2010年代半ばを過ぎるまで、バリ本島の人にとっても
   噂でしか知らない場所だった」という言い回しに弱め、特定の年号を落とした。
3. **北部が4都市だったのを5都市に増やした。**
   トゥジャクラ(北岸、アイル・サニーの湧水プールとアラック蒸留)を追加。
   これに伴い、路線は43→44本、5マス超だった3本のうちトゥランベン〜
   クブタンブハン(702px)がトゥランベン〜トゥジャクラ(526px)+
   トゥジャクラ〜クブタンブハンの2本に分かれ、**5マス超はロヴィナ〜
   ギリマヌッとタナロット〜ヌガラの2本のみ**になった(いずれも実在する
   一本道)。

最終: **33都市44路線**、4言語欠け0・mark/bg未使用0・連結性・物件価格帯を
再度機械確認済み(下記「自分で確かめたこと」は最終状態での結果)。
...
- `buildBaliContent()` が例外なく組み上がることを確認(`t()` は4言語に分けられないと
  ...
  結果: **都市33・路線44・クイズ36・アイテム9・厄災7・季節12・出来事20・
...
- `mark`/`bg` のキー突き合わせ: 33都市が参照する `mark`(12種)・`bg`(12種)が
...
- `sky()` の第3引数: 12背景すべてで「次に来る全面塗りの開始yと第3引数が
  ...
  無いことを確認済みです。
- 都市カードの1話1事実: 全33都市の `fact` を書く際、1文目に固有の事実
  (年号・数字・固有名詞のいずれか)、2文目に背景か現在の姿、という型を
  最初から統一して書いたため、後からの絞り込みは発生していません。
- 路線の連結性: 44路線が33都市すべてを1つの連結成分にまとめていることを
  機械確認(どの町からもどの町へもたどり着ける)。
- `seg` の実測: `BALI_PROJ` の式で44路線すべての投影後px距離を計算し、
  seg=110で最大8マス・5マス超2本(ロヴィナ〜ギリマヌッ・タナロット〜ヌガラ、
  いずれも実在する長い一本道)という結果を得て採用しました
  (詳細は `geography.mjs` と `cities.mjs` のコメント)。
- **物件価格の帯**: 64件(→トゥジャクラ追加で66件)を実測し、当初200〜320の
  1.6倍しか開いていなかったのを、`別格`2600〜2800・`高い`1200〜1700・
  `中`550〜850・`安い`150〜310の4段に振り直しました。**最終: 150〜2800
  (18.7倍)、利回りは全件20.65〜20.95%**(実測値)。振り直しの過程で、
  クタの「爆弾事件の慰霊壁」とクルンクンの「1908年ププタンの門」という
  2件が、悲劇の場そのものに値段と利回りを付ける形になっていたことに
  気づき、別の実在する物件(クタ・ビーチの遊歩道/プリ・アグン・スマラプラ)
  に差し替えました(史実そのものは `fact` の記述として残しています)。

## 登録後にteam-leadから回してもらった航路チェックの対応

登録後、`node scripts/check-sea-routes.mjs bali -v` で44路線中10本が60px超と
指摘を受けた。内訳と対応:

- **3本は本当に航路(`"sea"`)にすべきだった**(サヌール⇄ヌサペニダ・
  パダンバイ⇄ヌサペニダ・サヌール⇄ヌサレンボンガン)。最初の版で `"sea"` を
  書き忘れていた(3つ目の要素を落としていた)。追加した。
  **ヌサペニダ⇄ヌサレンボンガンは`"sea"`を付けなかった**
  (実測で陸路のまま0pxだったため。番付ける前に4通り試してから決めた)。
- **7本は道のままでよいが、海岸線が粗くて海にはみ出していた**
  (トゥジャクラ・トゥランベン・アメッド北東の岬、ブキット半島西岸、
  西岸のタナロット〜ヌガラ〜ギリマヌッにかけての一帯)。
  実在する町を含むよう `geography.mjs` の海岸線を6箇所張り出させ、
  端の入れ替え(添字はそのまま、ペア内の順序だけ変える)を6路線に適用して解消。
  **最終的に残ったのはヌサペニダ⇄サヌール(216px)とパダンバイ⇄ヌサペニダ
  (60px)の2本のみ**で、どちらも実在する船便について4通り(そのまま/端の
  入れ替え/陸路化/陸路化+入れ替え)すべて試した上でのいちばん小さい値
  (`cities.mjs` にコメントで残してある)。ヌサレンボンガン島がサヌール〜
  ヌサペニダの直線上にあるため、直線描画である以上これ以上は縮まらない。
  **KEPTとして残しているので、除外登録をお願いします。**
- ついでに `check-quiz.mjs bali` で見つかったクルンクン/ヌガラ絡みの
  重なり1件(答え「イスラム教」がヌガラのカードの「ヒンドゥー」と一致)を
  問いの言い回しを変えて解消した(ブドゥグル/ルピアの1件は前回同様、
  一般常識問題として残している)。
- `check-city-backgrounds.mjs` でバリ関連のNGは0件。
- `npm run shot -- bali overview` を実際に撮って目視確認。海岸線・都市配置・
  航路の点線・物件価格の桁(Rp1,200,000,000)すべて意図どおり描画されている。
  ラベルの重なりはteam-lead側の新しい検査(盤面からはみ出す名札の検査)に
  委ねる。
...
- **観光地への偏り**: team-lead指摘を受け、北部にトゥジャクラ(アイル・サニーの
  湧水プールとアラック蒸留)を追加し、南部6・東部6・北部5に揃えました。
