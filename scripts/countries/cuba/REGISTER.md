...
**倍率8000を提案しますが、確認をお願いします。**

キューバ・ペソ(CUP)は公式レートと非公式(市中)レートが大きく開いている
特殊な通貨です。

```
1ドル≒150円、1ドル≒120CUP(2024年の公式個人向けレート改定後)として
1CUP≒1.25円。12,000,000÷1.25÷1200=8000。
```

**別案(市中レート):** 非公式の市中レートは記事により1ドル=300〜400CUP
程度まで開いており、これを使うと倍率は21,000〜27,000程度になります。
どちらを使うかはウクライナ(2800倍)の例より振れ幅が大きい判断になるため、
**team-lead確認をお願いします。**確認いただいた値を以下に反映してください。

```js
  // $ 1,200 → $ 9,600,000。1ドル≒150円・1ドル≒120CUP(公式個人向け
  // レート、2024年改定後)として1CUP≒1.25円、12,000,000÷1.25÷1200=8000。
  cuba: 8000,
```
...
**キューバを北アメリカ・南アメリカどちらの束に入れるか、判断をお願いします。**
既存の束は大陸単位です(`northamerica: ["northamerica","canada","usa","mexico"]`、
`southamerica: ["southamerica","peru","venezuela","bolivia","brazil"]`)。
キューバは地理的には北アメリカ大陸(大アンティル諸島)ですが、既存の
どちらの束にも明示の「カリブ」枠はありません。**新しい束(`caribbean`)を
作るか、`northamerica` の `countryIds` にキューバを足すか**、決めてください。
束に入れなければ自動的に `other`(その他)扱いになりますが、それでよいかも
含めて確認をお願いします。
...
道具の値段  quiz-save(hojaderuta)は130(上限140以下)。gain-cash(cigarros)は280で、
            提案する効果額400より安い
通貨倍率    8000(提案。公式レートで計算。市中レートなら21,000〜27,000。要確認)
```
