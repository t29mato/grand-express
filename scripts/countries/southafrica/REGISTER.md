...
- `node scripts/check-sea-routes.mjs southafrica`(使い捨てcontent.jsonで実施、
  ...
  最終結果「60px超の食い違いなし」
- mark/bg: 当初41都市・記号37・背景40=計77枚。取りまとめ側の指示
  (記号+背景≤70枚・同じ絵になる都市≤4組)を受け、2026-08-20に背景を
  33種へ寄せて**記号37+背景33=計70枚**に。土地の性格が本当に同じ町
  (金鉱3件・ホームランドの首都2件・クルーガー周辺2件・石炭産業2件・
  東ケープの港2件)を背景共有にし、違いは記号で語らせた。結果として
  同じ絵になる都市は1組(ステレンボッシュ/フランシュフック。意図的。
  上限4組の範囲内)。詳細は `ART-KEYS.md` の該当節。
...
- 2026-08-20 JST: quiz.mjs を3問から101問に拡張。難易度分布は
  ...
  話)による誤検知と判断した。
- 2026-08-20 JST: `scripts/check-quiz-across-boards.mjs southafrica africa`
  を使い捨てcontent.json(`{id, quiz}` のみ、実施後に削除済み)で実施。
  **「1組、重なりなし」。**アフリカ盤との重複は事前に手で洗って3件
  (通貨=africa Q13・国歌の言語数=africa Q78・3つの首都=africa Q46)
  差し替え済みだったが、機械でも重なりが無いことを確認できた。
  **この道具は「答えの形が同じ」ものしか拾わない**(3つの首都のように
  同じ事実でも答えの形が違う――アフリカ盤は国名、こちらは都市名――場合は
  拾えないと取りまとめ側から指摘済みなので、手での突き合わせは今後も
  併用する)。**難易度9〜10の10問は、別の盤面の担当に裏取りを依頼したい。**
