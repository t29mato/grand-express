...
## team-leadレビューでの3点の修正(44都市に変更)

1. **ニューカッスルの重複を解消。** 北アイルランド枠の `mourne`(ニューカッスル、
   コ・ダウン)を `armagh`(アーマー)に差し替えた。二つの大聖堂(アイルランド教会と
   ローマ・カトリック、どちらも「アイルランド全島首座大主教」を名乗る)を核にした。
2. **ワイト島に本土側の港を追加。** `portsmouth`(HMSヴィクトリー・Dデイ出撃地)を
   se地方に新規追加。カウズとのあいだに `portsmouth-cowes`(航路)を張った。
3. **コーンウォールを追加。** `penzance`(セント・マイケルズ・マウント)を se地方に
   新規追加。`penzance-bristol`(陸路、グレート・ウェスタン線の延長)で接続。
4. **スタランレア→ケアンライアンに訂正。** `stranraer` を `cairnryan` に差し替え、
   実際に2011年以降ベルファスト航路が発着している港にした(スタランレアの歴史は
   ケアンライアンの豆知識の中で触れている)。

都市数は42→**44**(se10→12、他地方は変更なし)。中部6はそのまま(指示どおり
既存地方を削って埋めることはしていない)。座標変更3件(cairnryan・ポーツマス沖の
海岸線バッファ)を含め、下記「自分で確かめたこと」を全項目再実行して確認済み。
...
- `node -e 'import("./scripts/countries/uk/index.mjs").then(m => { const c = m.buildUkContent(); console.log(Object.keys(c.cities).length, c.edges.length, c.quiz.length); })'`
  → `44 58 40`(都市44・路線58・クイズ40)。エラーなし。
- **4言語の書き漏らし**: `buildUkContent()` の出力全体を再帰的に walk し、
  `{en,es,fr,ja}` の形をした値をすべて機械的に検出して空文字が無いか確認するスクリプトを
  自作して実行 → **558件すべて4言語とも非空**(韓国で起きた「日本語だけ33都市分欠落」の
  パターンは無いことを確認済み)。
- **表示名の重複**: 44都市の英語名を突き合わせ、重複ゼロを確認(今回の指摘の
  ニューカッスル重複を含め、以後同種の穴が無いことを機械的に確認)。
...
- **海岸線と都市の位置**: `use-board-layout.test.ts` と同じ point-in-polygon 判定を
  自作スクリプトで再現し、44都市全件が陸の上にあることを確認(海岸線に近い都市は
  最短で4.9px、多くは10px以上の余白)。ポーツマス(海岸線頂点と座標が完全一致していた)と
  ケアンライアン(ロッホ・ライアン湾口の頂点まで0.9pxしか無かった)の2件は、
  海岸線側を沖へ押し出して直した。
- **路線の陸海判定**: `check-sea-routes.mjs` と同じ式(octilinear の折れ点・land判定)を
  自作スクリプトで再現し、58路線のうち陸海の食い違いが60px超のものを検出 → 端の入れ替えで
  9件を解消。残る2件は意図的に残した:`holyhead-belfast`(418px中88px=21%がアングルシー
  北端をかすめる、韓国のjeju-yeosuと同じ理由)と `penzance-bristol`(591px中115px=19%が
  ブリストル海峡沿いの湾をかすめる、デヴォンに中継駅を置いていない長距離線のため)。
- **都市どうしの近さ**: `use-board-layout.test.ts` と同じ式で、押し離しても重なる
  組が無いことを確認(0件)。
- **路線の連結**: 44都市すべてが1つの連結成分に含まれることを確認(孤立都市0件)。
...
- **背景の密度**: 22背景の平均68.4要素(`<rect|circle|ellipse|line|path|polygon|polyline|g`
  ...
  ほぼ一致したので数え方の妥当性を確認済み)。最小は39(`linksgolf`)。ポーツマス・
  ペンザンス・アーマー・ケアンライアンは既存の `mark`/`bg` キーを再利用したので
  art.mjsの変更は無し。
