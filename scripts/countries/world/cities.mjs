...
export const WORLD_EDGES = [
  ...
  // ヨーロッパ・大西洋まわりで地球を反対に回るしかなかった。
  //
  // 盤面は経度 -188〜216 で切ってあるので、**太平洋は左右の端に分かれている**
  // (スバ x=3358 / パペーテ x=352)。まっすぐ引くと3000pxの線が盤面を横切り、
  // 南アメリカとアフリカを串刺しにする。そこで、この路線だけは
  // **日付変更線で切って両端から描く**(`use-board-layout.ts` の `wrapX`)。
  // 経度でつなぎ直すと 178.44°E から 210.4°E で、実距離はフィジー—タヒチの
  // 3,200km。太平洋の島伝いとして無理がない。
  ["suva", "papeete", "sea"],
];
