...
- `node -e 'import("./scripts/countries/ghana/index.mjs").then(m => { const c = m.buildGhanaContent(); console.log(Object.keys(c.cities).length, c.edges.length, c.quiz.length, c.moneyEvents.length); })'`
  → `38 42 38 22`(都市38・路線42・クイズ38・出来事22。ケテ・クラチ追加で
  都市37→38・路線41→42)。例外なし(`buildGhanaContent()` は全 `t()` 呼び出しを
  通すため、4言語のどれか1つでも欠けていれば例外で落ちる。実際に作業中、
  quiz.mjsで1件〈PANAFESTの設問の日本語訳を書き忘れ〉ここで落ちて気づけた)。
- 4言語欠けの機械チェック: `cities.mjs`(38都市×n+props)・`flavour.mjs`
  ...
  実行し、0件を確認(ケテ・クラチ追加後も再確認済み)。
- 都市の座標: 全38都市が投影範囲内(`LON0<lo<LON1`・`LAT1<la<LAT0`)かつ
  海岸線ポリゴン(`GHANA_LAND[0]`)の内側にあることをpoint-in-polygonで
  機械確認済み(38件とも `true`)。海岸沿いの都市はいずれも海岸線から
  緯度でおよそ0.12〜0.16度(≒39〜52px)の余裕を持たせてある(実測)。
- `mark`/`bg`: 38都市すべての参照先(15キー)が `GHANA_MARKS`/`GHANA_BG` に
  存在することを機械確認済み(不足0)。ケテ・クラチは`hills`を再利用
  (クパンドと同じ「丘の町が湖に沈んで湖畔の港になった」筋なので流用が自然)。
- `seg` の実測: 42路線すべての投影後距離を実測。既定の84だと3本
  (最長939px、クマシ―タマレ)が5マス超になったため、110にして
  クマシ―タマレ1本(939px→9マス、上限クランプ)だけが5マス超に収まる
  ことを確認した(ケテ・クラチ追加後も、新しい2区間〈490px→4マス・
  427px→4マス〉はいずれも5マス以下で再確認済み)。
...
- `mark` と `bg` はキーを1対1で揃えた(イタリアのように別集合にはしていない)。
  38都市に対し15種(1種あたり平均2.53都市)で、イタリア(27種/45都市≒1.67)
  より粗いが、韓国(16種/40都市=2.5)に近い密度。
- 路線42本/38都市≒1.11本/都市。イタリア(59/45≒1.31)よりやや疎ら。
  北部8都市(タマレ・イェンディ・ボルガタンガ・ワ・パガ・ララバンガ・
  サラガ・イェジ)は鉄道が無いため道路だけで疎に結んであり、単一路線の
  末端になっている町(サラガ・イェジ・パガの一部)がある。全体は
...
- **厄災の効果割り当て(6b)**: 7種の効果(fine/percentLoss/skipTurn/
  ...
  あれば直してほしい。
- **ボルタ湖の渡し(アコソンボ⇄イェジ)の航路 → 対応済み**: team-leadが
  検査側を直したところ98%が陸のままだったため、実在の中継港ケテ・クラチ
  (-0.05, 7.7667。ドイツ領トーゴランド時代の行政拠点で、ダムの湛水で
  旧市街が水没し住民ごと高台へ移された町。38番目の都市として`vol`地方に
  追加)を挟んで`akosombo-ketekrachi`・`ketekrachi-yeji`の2区間に割った。
  投影後の距離を実測し、湖の楕円チェーンとの距離が直線1本のときの
  15〜97px(8サンプル中6件が40px超)から、2区間に割った後は
  -45px〜19px(ほぼ全サンプルが楕円の内側〜ごく近傍)まで縮まることを
  確認した。`sea`のまま2本とも維持している(陸路にしていない)。
...
- **「オワレ賭博の負け」の言い方**: オワレはマンカラ系の盤上遊戯であって
  賭博そのものではない、という指摘を受け、`flavour.mjs`の`owarebet`を
  書き直した。タイトルを「オワレの勝負に負ける」→「賭けたオワレの一局に
  負ける」に変え、本文も「二人の対局者が種を蒔く前にささやかな賭け金で
  合意していた」→「オワレそのものは西アフリカ中で打たれるただの盤上遊戯」→
  「この一局を賭け事に変えたのは脇に置かれた硬貨であって遊戯そのものでは
  なかった」という順で、賭けたのは人であって遊戯の性質ではないと分かる形にした。
- **「ファンタジーコフィンの葬列渋滞」の言い方**: 棺を珍しい見世物として
  紹介しないという指摘を受け、同じく`owarebet`の下の`fantasycoffin`を
  書き直した。カカオの実の形の棺(誰の棺か分からない書き方)から、
  「生涯乗ってきた船をかたどった棺に納められて最後の眠りへ運ばれる漁師」
  という、送られている人がいることが分かる書き方に変えた。「美術館に売られる」
  という見世物寄りの結びも削り、「それぞれが弔うべき生涯の形に合わせて作る」
  という工芸の目的で締めた。厄災の絵(`ghana-fantasycoffin.tsx`)も合わせて、
  カカオの実の形から魚の形の棺に描き直し、4人の担ぎ手を明示的に描いた
  (以前は担ぎ手が見えず、棺だけが浮いているように見えた)。
- **アナンシの物語がカリブ・南北アメリカへ渡った角度**: 提案を受けて
  `GHANA_SPIRIT.wakeFact`に1文足した。「アナンシは奴隷にされたアカンの人々の
  記憶に乗って大西洋を渡った。物語はどんな持ち物とも違うやり方で持ち運べた
  からである。同じ知恵者はいまもジャマイカではアナンシ、スリナムではアナンシ、
  アメリカ南部ではアント・ナンシーと呼ばれている」という内容(史実として
  確立している民俗学の知見なので、伝聞扱いにはしていない)。
- **背景を1枚ずつ40個に**: 平均だけでなく15枚**すべて**が40個以上になる
  よう`art.mjs`を作り直した。修正前は最少25(capital/savanna/goldmine)・
  平均30.0だったが、修正後は**最少40(hills/kente/goldmine)・最多52
  (mosque)・平均44.7**。`rsvg-convert`でPNG化したうえで、`sharp`で
  マゼンタ(#ff00ff)ピクセルを全画素走査する機械チェックを15枚全部に
  実施し、塗り残し0を確認した(目視だけでなく画素単位で確認したのは今回が初めて)。

修正後、`node -e`で`buildGhanaContent()`が例外なく通ることを再確認し、
`scripts/countries/ghana/`と`dooms/ghana-*.tsx`のみをコミットする。
