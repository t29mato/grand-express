...
47都市(team-lead指摘によりプエルト・エデンを追加、2026-08-21)、`mark` 46種・
`bg` 24種(土地の性格が同じ町でまとめた)。**記号46+背景24=70枚**(取りまとめ側の
上限70枚ちょうど)。プエルト・エデンだけはプエルト・アイセンとmark/bgを
共有しているため、**同じ絵になる都市は1組**(47都市の上限4組の内側)。
...
'
# 都市 47 / 記号 46 / 背景 24 → 計 70 枚 / 同じ絵になる都市 1 組(プエルト・アイセン/プエルト・エデン)
```
...
| au | プエルト・エデン | `riverport`(プエルト・アイセンと共有) | 同上。水上飛行機の発着場と孤絶した集落 |
...
| `patagoniasteppe` | コジャイケ・プエルト・アイセン・プエルト・エデン・チレ・チコ・コクラン | パタゴニアのステップ。道が途切れる開拓地 |
...
ナバリノ島(`NAVARINO`)・ウェリントン島(`WELLINGTON`)の4ポリゴン。ナバリノ島
(プエルト・ウィリアムズ)とウェリントン島(プエルト・エデン)はどちらも
都市が1つだけなので、手で輪郭を引かず**その都市の座標を中心にした楕円**に
してある(オセアニア盤と同じ手法)。絵を描く際もこの1都市1島の関係を
崩さないこと。
