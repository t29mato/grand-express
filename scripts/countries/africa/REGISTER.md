...
都市63・路線68・クイズ103・お金の出来事18・アイテム9・厄災7・季節12・
mark18種・bg17種・音楽7地方。以下、共有ファイルへ貼り付けるためのコード片。

(60都市承認後、`toamasina`・`bamako`・`ouagadougou`を追加して63都市に
なった経緯は末尾「迷った点」参照。数値はすべて最終版。)
...
- `node -e "import('./scripts/countries/africa/index.mjs').then(m => { const c = m.buildAfricaContent(); console.log(Object.keys(c.cities).length, c.edges.length, c.quiz.length, c.moneyEvents.length, Object.keys(c.items).length, c.doom.length, c.seasons.length, Object.keys(c.marks).length, Object.keys(c.bg).length, Object.keys(c.styles).length); })"`
  → `63 68 103 18 9 7 12 18 17 7`。例外なし。
...
- **陸地判定**: `world/geography.mjs` の `AFRICA`/`MADAGASCAR`/`ZANZIBAR`
  ポリゴンで63都市すべてが陸地内・投影範囲内であることを確認した。
...
- 都市1件あたりの面積: 63都市・BW2268×BH2382=5,402,376px² →
  約85,752px²/都市(目安90kにほぼ一致)。
- 路線密度: 63都市に対して68本(1都市あたり1.08本)。全体で1つの
  連結成分になっていることを確認済み。
- 国の内訳: 35か国(バマコ=マリ・ワガドゥグ=ブルキナファソの追加で
  33→35)、1国につき1〜3都市(上限厳守)。
...
- **サヘル地域の判断を、国ごとではなく町ごとに切り直した。**
  当初は「マリ・ブルキナファソ・ニジェールには都市を置かない」としていたが、
  team-lead指摘を受けて改めた。北部マリ(トンブクトゥ・ガオ)、
  ブルキナファソ北部・東部、ニジェールのティラベリ/リプタコ・グルマ
  三国国境地帯は実際に危険なので引き続き置かないが、**バマコ(マリの
  首都)・ワガドゥグ(ブルキナファソの首都)は、そこで人が普通に暮らし
  列車も走っている町**として62・63都市目に追加した(35か国目・国数が
  33→35に増えた理由)。ワガドゥグは、アビジャン〜ワガドゥグ(シタレール)
  が「貨物(年間およそ100万トン、カカオ・綿・カシューナッツ・マンガン鉱)
  は今も動いているのに、旅客は2020年のコロナ国境封鎖以来ずっと止まった
  まま(2024年6月時点で運行なし)」という、大陸盤面の主題そのものの
  題材になったため採用し、アビジャンの `fact` もこの事実に合わせて
  書き直した。チャドのンジャメナは、紛争ではなく「鉄道が一度も届いて
  いない」空白を主題にできる首都として引き続き `sah` 枠に採用している。
- **厄災の神を、特定の一国の民話ではなく「完成しなかったケープ〜カイロ
  鉄道」そのものにまつわる噂にした。**63都市・35か国にまたがる盤面で
  一国の民話を大陸全体の伝承であるかのように語るのは不誠実だと判断し、
  アジア盤面の「時刻表に無い列車」と同じ扱い方を採った。
- **難易度9のンクルマの問い、解説文(`f`)の死去した場所を直した。**
  peer-reviewで見つかった誤り。「ギニアに亡命したまま死去」ではなく、
  「ギニアに亡命したが、当地では受けられない癌治療のためブカレストへ
  送られ、そこで死去」が正しい。問い・選択肢・正解には影響しない。
- **`dhowpassage`(向きの選べない移動、`carried-far`)の価格を300→260に
  下げた。**同じ「操作できる移動」枠の `coffeesack`(`roll-fixed-dice`、
  300)と同額だったため、「操作できない移動は、操作できる移動より安く
  する」規則(メキシコ盤面での違反例が根拠)に合わせた。
- `walvisbay` の背景を `sahara` から `desertport` に変えた。港町としての
  性格のほうが素直だという絵の担当の助言を採用した(既存キーの再割当て
  のみで、新しい鍵は増やしていない)。
