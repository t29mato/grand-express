...
- **路線のgeometry**: `check-sea-routes.mjs` を自分で実行できました
  ...
  **最終的に0本**まで減らしました(2026-08-20、22:xx JST時点で確認)。
  - `quangngai`–`quynhon`(トゥ゙ンク線本体、47〜51%が海の上): 端の入れ替え
    (`["quynhon","quangngai","rail"]` の順に変更)で0pxに解消。**添字は
    動かしていない**ので、他の路線への波及はありません。
  - メコンデルタ・ホーチミン〜ヴンタウの6本(いずれも「航路が陸の上・100%」):
    このデルタは川筋を除けばほぼ全域が陸地で、開けた海面が無いため、
    "sea" のままでは検査に通りませんでした。**陸地ポリゴンに実際の
    川筋の水面を切り込む作業までは今回手を入れず**、`hochiminh`–`mytho`・
    `mytho`–`cantho`・`cantho`–`chaudoc`・`cantho`–`camau`・
    `cantho`–`rachgia`・`hochiminh`–`vungtau` の6本を、ディエンビエンフー
    等と同じ「実在しない/水路上の移動をゲーム上の連結のために抽象化する」
    扱いで `"rail"` に変更しました。**都市カードのfactでは、この6都市に
    実在の鉄道が無いことをそのまま伝えています**(ゲームの線の見た目と
    文章の内容が食い違う点として、承知のうえの判断です。より正確を
    期すなら、デルタの川筋を水面ポリゴンとして切り込む追加作業を
    どなたかにお願いしたいです)。`mytho`–`bentre`・`rachgia`–`phuquoc`
    (実在するフェリー航路)は "sea" のまま問題なく通りました。
...
- **通貨倍率1,650,000は想定為替レートに基づく概算です。**実際のレートで
  検算をお願いします。
- **メコンデルタ・ヴンタウの6路線を"sea"から"rail"に変えた判断**(上記
  「自分で確かめたこと」参照)。都市カードの文章では実在の鉄道が無いと
  明記しているため、絵と路線表示・文章のあいだに食い違いが残ります。
  デルタの川筋を陸地ポリゴンに切り込む追加作業をするか、この抽象化の
  ままでよいか、判断をお願いします。
- **クイズ難易度9〜10のうち2件(コイン流通停止の年、森林被覆率)は
  確度中程度と自己申告します。**他盤面の担当に裏取りをお願いしたいです。
