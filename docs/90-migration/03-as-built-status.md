...
| Phase 7: テスト強化 | ✅ 完了 | ユニット/コンポーネントテスト192件、E2E(Playwright)10件(起動〜プレイ、多言語、セーブ/ロード、アクセシビリティ、モバイル幅、実乱数での連続プレイ稼働テスト)。カバレッジはDomain/Applicationともに目標達成。稼働テストで実バグ(駒がクリックを妨げる問題)を発見・修正済み |
| Phase 8: 演出/音声/パフォーマンス仕上げ | 🔶 大部分完了 | **完了**: バンドルサイズ改善(国コンテンツの動的import化、最大チャンク約620KB→約366KB)、駒移動のCSSトランジション、盤面カメラのパン/ズーム追尾(手番プレイヤーへの自動追尾+全体表示トグル)、地方ごとの和音BGM(legacyの実際のコード進行データを使用)。**未着手**: 拍単位のフル音楽スケジューリングエンジン(メロディ/ドラム/ストラム、legacyの`Snd`相当)、3Dダイスの物理的なバウンド演出、現行版との厳密なピクセル単位のビジュアル突合QA(これらは人が実際に見聞きしながらの反復調整が本質的に必要な領域) |
| Phase 9: 移行カットオーバー | 🔶 一部完了(デプロイはオーナー判断待ち) | 9.2最終受け入れ確認は下記チェックリストの通り実施。9.3(旧版のアーカイブ)は完了(`legacy/`配下)。9.1(本番デプロイ)はホスティング先の選定がプロジェクトオーナーの判断事項のため未実施。手順は [04-deployment-guide.md](./04-deployment-guide.md) に用意済み |
...
| # | 完了の定義 | 状況 |
|---|---|---|
| 1 | 現行の全機能(2ヶ国・4言語・全アイテム/クイズ/季節イベント/厄災効果)が新アプリで再現できている | 🔶 **機能面はほぼ達成**: 2ヶ国の実データ(都市30・アイテム9・クイズ・季節12・厄災7)、4言語、全ゲームメカニクス(移動・売買・クイズ・アイテム効果・厄災・CPU)を実装・実データでテスト済み。**演出面は簡略化**(上記「実装時に行った主な簡略化」参照)しており、原作と見た目・音が同一ではない |
...
| 5 | 本番相当環境にデプロイ済みで、旧HTMLの提供は停止(またはアーカイブ)している | 🔶 アーカイブは完了。デプロイはオーナー判断待ち(未実施) |

**総括**: 機能・アーキテクチャ・テストの観点では移行はほぼ完了しているが、(a)演出/音声の作り込み、
(b)本番デプロイの2点は、それぞれ「反復的な人による確認が必要な作業」「ホスティングに関する
オーナー判断」であり、自動化された開発作業だけでは完了させられない性質のものとして残っている。
...
4. **盤面の視覚表現**: 都市ごとの手描きSVGイラストと中間マスの湾曲配置(jitter)は実装していない
   (直線配置の抽象的なノード/エッジ表示)。カメラのパン/ズーム追尾は実装済み(`use-camera.ts`)。
5. **音声**: 拍単位のフル演奏エンジン(メロディ/ドラム/ストラム)は未実装。地方ごとの和音を
   通過時に1回鳴らすアンビエント表現(`region-music-table.ts`、legacyの実データを使用)と
   短いトーンによる効果音を実装済み。
