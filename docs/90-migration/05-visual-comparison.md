...
- **盤面の地形イラスト**(都市ごとの手描きの地形を盤面そのものに敷く表現)は未実装。
  盤面は引き続き円形ノード+直線エッジの抽象表示(ADR-0007に記録済み)。
  都市カード/モーダル側のイラストは移植済みなので、残るのは盤面のみ。
- 国選択カードのポリゴン地図(legacyの `countryThumb()`)は未実装。
  こちらも同様に決定的な関数なので、必要になれば同じ手法(抽出して静的データ化)で移植できる。
