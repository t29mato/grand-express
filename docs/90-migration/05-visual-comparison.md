...
- legacyはゲーム開始時に到着都市(La Paz)の手描き風イラスト付き「出発ストーリー」モーダルが
  表示される(山・雲・ケーブルカー・街並みのSVGイラスト)。新アプリはこの導入ストーリー
  モーダル自体を実装していない(初手からいきなり操作可能な盤面が表示される)。
...
- 到着ストーリーモーダル(ゲーム開始時・目的地到着時の演出)自体を実装するかどうか。
  現状はゲームロジック(`arriveAtDestination`ユースケース等)は完全に再現しているが、
  演出だけが省略されている。
