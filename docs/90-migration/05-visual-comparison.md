...
- ~~盤面の凡例~~ → **対応済み(2026-08-05)**(`board-legend.tsx`)。
...
- ~~路線(レール)の3層描画と盤面上の都市ノードのシンボル表示~~ → **対応済み(2026-08-05)**。
  路線は暗い縁取り→明るいレール→枕木のダッシュの3層、都市ノードは影+シンボル+円+
  中心の点+目的地リング+ラベル(左/右/下の位置指定つき)、中間マスは角丸の四角+記号
  (`+`/`−`/`★`/`?`、クイズは難易度別の色)で、いずれもlegacyと同じ色・寸法で描画する。
...
- 2026-08-05: 路線の3層描画・都市ノードのシンボル・中間マスの記号も移植。
- 2026-08-05: 盤面右下の凡例も移植。盤面全体の描画結果は
  `assets/new-board-overview.png` を参照(地形・都市シンボル・レール・地名・凡例が揃った状態)。
