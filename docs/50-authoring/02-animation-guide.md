...
`npm run preview` は以前、JSXから `<svg>…</svg>` を文字列として切り出して描いていた。
そのため **`{list.map(...)}` で作る要素が描かれないまま静かに消え**、
紙吹雪や後光がごっそり抜けた絵を「確認できた」と誤認していた。

いまは Vite で本当にコンポーネントを組み立てて描いている。
**繰り返しで作る部分がちゃんと出ているか、必ず目で確かめること。**

## よく踏む落とし穴

**CSSキーフレームの `transform` は、SVGの `transform` 属性を上書きする。**
同じ要素に位置決めの `transform="translate(...)"` とアニメーションの両方を付けると、
アニメーションが始まった瞬間に原点へ飛ぶ。

  位置決めは外側の `<g transform>`、動きは内側の要素のクラス、と分けること。

**変形させる要素には `transform-box: fill-box` と `transform-origin` を必ず書く。**
書かないと、拡大・回転がSVG全体の原点を基準に効いてあらぬ方向へ飛ぶ。
