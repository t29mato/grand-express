...
- **現状**: Domain/Application/Infrastructure/Presentationの全層を実装し、実際にプレイできる状態まで完了(ユニット/コンポーネントテスト194件・E2Eテスト10件がグリーン)。バンドルサイズ改善、音楽エンジン・3Dダイス演出のフル移植など仕上げも完了。残るのは本番デプロイ(ブラウザ認証を要するオーナー判断待ち)と、手描きイラスト等どこまで似せるか人の判断が必要なビジュアル仕上げのみ。詳細は [as-built](./90-migration/03-as-built-status.md) を参照。
