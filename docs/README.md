...
- **現状**: Domain/Application/Infrastructure/Presentationの全層を実装し、実際にプレイできる状態まで完了(ユニット/コンポーネントテスト192件・E2Eテスト10件がグリーン)。バンドルサイズ改善などパフォーマンス対応も一部実施済み。演出・音楽の作り込みと本番デプロイ(オーナー判断待ち)は今後の対応として残っている。詳細は [as-built](./90-migration/03-as-built-status.md) を参照。
