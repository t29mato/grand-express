...
| 90-03 | [as-built(完了状況)](./90-migration/03-as-built-status.md) | 各フェーズの実施結果・簡略化点・残タスク・ファイルサイズ削減効果 |
...
- **現状**: Domain/Application/Infrastructure/Presentationの全層を実装し、実際にプレイできる状態まで完了(ユニット/コンポーネントテスト191件・E2Eテスト7件がグリーン)。演出・音楽の作り込みと本番デプロイは今後の対応として残っている。詳細は [as-built](./90-migration/03-as-built-status.md) を参照。
