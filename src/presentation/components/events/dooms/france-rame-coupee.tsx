 ...
 * 前の半分は本線を右へ、乗ってしまった後ろの半分は分岐へ。
 * 窓から顔を出した旅人が、離れていく前半分を見送っている。
 *
 * ## 車両を傾けるのをやめた
 *
 * 後ろ半分は分岐の勾配に合わせて13度傾けてあった。**分岐線の絵に合ってはいた。**
 * ところが2026-08-09、二人が別々に見て**二人とも脱線事故だと読んだ。**
 *
 * 決め手は**もう一方が水平に走っていること**だった。同じ絵の中に対比があるので、
 * 「遠近で手前に来ている」とは読めない。**片方だけ傾いていれば、それは倒れた車両。**
 * ガイドの「破壊そのものを描かない」に当たる。
 *
 * そこで、
 *
 * - 後ろ半分を**水平**に戻し、下へずらして置いた
 * - 分岐線を**手前に来る線**として描き直した。左下へ行くほどレールの間隔を広げ、
 *   枕木を長くしてある。**下がっているのではなく、近づいている。**
 *
 * 横から見た絵で線路が15度下るのは、そもそも鉄道としてありえない勾配だった。
 * **絵に合わせて車両を傾けたのではなく、線路のほうが間違っていた。**
 *
...
export function FranceRameCoupee() {
  return (
    <svg viewBox="0 0 400 210" role="img" aria-hidden="true">
      ...
      {/* 分岐して**手前に**来る線。左下へ行くほどレールの間隔を広げる。
          下っているのではなく、近づいている。 */}
      <g fill="#6b727a">
        <path d="M196,126 q-104,20 -196,44 l0,6 q94,-24 196,-44z" />
        <path d="M196,150 q-104,34 -196,66 l0,7 q94,-34 196,-67z" />
      </g>
      {/* 枕木。手前ほど長く、太くする。 */}
      <g fill="#3a322a">
        ...
        <rect x="150" y="140" width="10" height="42" />
        <rect x="96" y="150" width="12" height="52" />
        <rect x="38" y="160" width="14" height="62" />
      </g>
      {/* 転轍機 */}
