...
function relaxOverlaps(
   ...
   * 見えてしまう。
   * 中間マスには上限をかけない——湾を越える路線の上など、海の上にあってよい。
   *
   * **陸から出ないことを担保しているのは、この上限ではない。**下の `move` が
   * `isOnLand` を直に見て、海へ出る移動そのものを断っている。ここは
   * 「地理を大きく崩さない」ためのもので、安全側の仕掛けは別にある。
   *
   * **かつてこのコメントには「海岸線は terrain-layer.tsx の COAST_BUFFER で
   * 外へ広げてあるので、その範囲なら陸から出ない」と書いてあった。二重に
   * 間違っている。**その名前の定数は terrain-layer.tsx に存在せず、
   * 陸を一律に外へ広げる処理は**やめている**(関門海峡が埋まって本州・四国・
   * 九州が地続きになったため。`use-board-layout.test.ts` に経緯がある)。
   *
   * そして**この上限は陸の大きさと無関係である。**2026-08-21、オセアニア盤で
   * 上限12.0px に対して**島の半径が3〜6px**という組み合わせが出た。
   * 島より大きく動かせる。**「上限内なら陸の上」という理屈は、島が上限より
   * 大きいときにしか成り立たない。**
   */
