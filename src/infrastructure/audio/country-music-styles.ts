...
const STYLE_LOADERS: Readonly<Record<string, () => Promise<unknown>>> = {
  ...
  oceania: () => import("../content/oceania.content.json").then((m) => (m.default as { styles: unknown }).styles),
  ...
  kyushu: () => import("../content/kyushu.content.json").then((m) => (m.default as { styles: unknown }).styles),
