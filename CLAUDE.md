...
- ノートは変更履歴ではなく**遊ぶ人から見て何が変わったか**を書く。
  ...
  出来事が起きるようになりました」。
- **版を上げるのは「出す直前」ではなく「その変更を書き終えたとき」。**
  2026-08-15、携帯で1画面に収める直しを PR で出したとき、版もノートも付けずに
  マージされ、**遊ぶ人から見て変わったのに v0.45.0 のまま本番に出た。**
  あとから 0.45.1 で補ったが、出た直後の版番号は嘘をついていたことになる。
  **PR を出す時点で版とノートを含めておく。**マージする人が気づく前提にしない。
- **出したあとは3つのURLを全部開いて版番号を読む。**
  `grand-express` は main への push で自動追随するが、
  `world-express` と `altiplano-express` は手で張り替えないと取り残される
  (実際に2版ぶん置いていかれていた)。
